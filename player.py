...
class Player(pygame.sprite.Sprite):
	...
	def input(self):
		...
		if keys[pygame.K_s] and self.can_slide and not self.is_damage:
		...
		if keys[pygame.K_w] and self.on_floor and not self.is_damage:
		...
		if keys[pygame.K_CAPSLOCK] and self.on_floor and not self.is_moving and not self.is_damage:
			...
			y_offset = vector(30, 0) if self.status.split("_")[0] == 'left' else vector(-30, 0)
		...
		# Projectile attack
		if keys[pygame.K_TAB] and self.can_shoot and not self.slide and not self.melee and not self.is_damage:
	...
	def slide_check(self):
		...
		if current_time - self.slide_timer >= 3000 and not self.can_slide:
			self.can_slide = True
		...
		for sprite in self.projectile_sprites.sprites():
			...
			if sprite.rect.colliderect(self.rect) and not self.slide:
				...
				if self.rect.x >= x:
					...
					self.is_damage = True
					self.damage += 3
					self.direction.x = 0.05 * self.damage
